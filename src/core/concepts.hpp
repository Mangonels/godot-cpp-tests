...
namespace ns::inline utils {
    ...
    template <typename T>
    concept GDObjectDerived = std::derived_from<std::remove_cvref_t<std::remove_pointer_t<T>>,
                                                godot::Object> ||
                              std::same_as<std::remove_cvref_t<std::remove_pointer_t<T>>,
                                           godot::Object>;
    ...
    concept VariantCompatible = VariantConstructable<std::remove_cvref_t<T>> &&
                                VariantConvertable<std::remove_cvref_t<T>> && requires {
                                    godot::GetTypeInfo<std::type_identity_t<std::remove_cvref_t<T>>,
                                                       void>::VARIANT_TYPE;
                                };
}
